...
# Loads your handle from the hidden .env file
load_dotenv()
...
def get_attempted(handle):
    try:
        ...
        print(f"Blacklisted: {len(attempted_ids)} unique IDs, {len(attempted_names)} unique Names.")
...
def start_simulated_contest():
    print("\n--- Initiating Focused Grind (1300 + 1400) ---")
    
    # Strictly one 1300 and one 1400
    ratings = [1300, 1400]
    ...
    try:
        ...
        if data.get('status') != 'OK':
            print("Global Problemset API Status Failed.")
            sys.exit(1)
    ...
    if len(selected) < 2:
        print("Warning: Could not find enough valid problems to fill both slots.")
    ...
    <h2>Codeforces Focused Grind</h2>
    """
